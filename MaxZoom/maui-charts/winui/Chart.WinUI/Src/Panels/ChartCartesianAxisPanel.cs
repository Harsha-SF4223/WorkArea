...
namespace Syncfusion.UI.Xaml.Charts
{
    ...
    internal class CompositorLinesPanel : Panel
    {
        ...
        public CompositorLinesPanel(Style style)
        {
            ...
            strokeToken = line.RegisterPropertyChangedCallback(Line.StrokeProperty, OnStrokeChanged);
            StrokeThicknessToken = line.RegisterPropertyChangedCallback(Line.StrokeThicknessProperty, OnStrokeChanged);
        }
        ...
        //WINUI-859906 To update the tick line size due to an application crash when navigating the page that contains the axis-based chart using the navigation view item tag.
        internal void UpdateSize(Size size)
        {
            if (shapeVisual != null)
                shapeVisual.Size = new Vector2((float)size.Width, (float)size.Height);
        }
        ...
        internal void Dispose()
        {
            if (line != null)
            {
                line.UnregisterPropertyChangedCallback(Line.StrokeProperty, strokeToken);
                line.UnregisterPropertyChangedCallback(Line.StrokeThicknessProperty, StrokeThicknessToken);
            }
