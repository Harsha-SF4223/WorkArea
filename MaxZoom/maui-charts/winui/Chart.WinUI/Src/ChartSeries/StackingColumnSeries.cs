...
namespace Syncfusion.UI.Xaml.Charts
{
    ...
    public class StackedColumnSeries : StackedSeriesBase, ISegmentSpacing
    {
            ...
            foreach (ChartSegment segment in Segments)
            {
                ...
                if (!double.IsNaN(elementSize))
                {
                    ...
                    if (this.AdornmentsInfo != null && AdornmentsInfo.Visible)
                    {
                        ...
                        // WinUI-845023 Removed the keyframe coding section to avoid debugger exceptions during chart maximization, we have animated data labels after data points animation has been completed.
                        label.Opacity = 0;
                        ...
                        DoubleAnimation animation = new DoubleAnimation()
                        {
                            From = 0,
                            To = 1,
                            Duration = new Duration().GetDuration(TimeSpan.FromSeconds((AnimationDuration.TotalSeconds * 30) / 100)),
                            BeginTime = TimeSpan.FromSeconds(AnimationDuration.TotalSeconds)
                        };
        ...
        internal override void BindStrokeProperty(DependencyObject segment)
        {
            if (segment is ChartSegment)
            {
                Binding strokeBinding = new Binding();
                strokeBinding.Source = this;
                strokeBinding.Path = new PropertyPath("Stroke");
                BindingOperations.SetBinding(segment, ChartSegment.StrokeProperty, strokeBinding);
            }
            else if (segment is ChartLegendItem)
            {
                Binding strokeBinding = new Binding();
                strokeBinding.Source = this;
                strokeBinding.Mode = BindingMode.TwoWay;
                strokeBinding.Path = new PropertyPath("Stroke");

                BindingOperations.SetBinding(segment, LegendItem.StrokeProperty, strokeBinding);

                strokeBinding = new Binding();
                strokeBinding.Source = this;
                strokeBinding.Path = new PropertyPath("StrokeWidth");
                strokeBinding.Mode = BindingMode.TwoWay;
                BindingOperations.SetBinding(segment, LegendItem.StrokeThicknessProperty, strokeBinding);

            }
            base.BindStrokeProperty(segment);
        }
