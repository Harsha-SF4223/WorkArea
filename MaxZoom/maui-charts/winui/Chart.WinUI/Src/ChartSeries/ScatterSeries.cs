    ...
    public class ScatterSeries : XyDataSeries
    {
        ...
        internal override void Animate()
        {
            ...
            if (sb != null)
            {
                if (!canAnimate)
                {
                    // WinUI-845023 To prevent data points from disappearing when maximizing the bubble chart, we have replaced the Stop method with SkipToFill in the storyboard.
                    sb.SkipToFill();
                    if (ShowDataLabels)
                    {
                        // WinUI-837121 Remove the clear value method for the render transform property of the framework element due to an unhandled debugger issue
