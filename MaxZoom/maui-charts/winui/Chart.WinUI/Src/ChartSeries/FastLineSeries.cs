...
namespace Syncfusion.UI.Xaml.Charts
{
    ...
    public class FastLineSeries : XyDataSeries
    {
        ...
        {
            // Need to fix the TooltipInteraction, temporary fix for WinUI-830184 WinUI Crashed issue when mouse hover on the series while data points dynamically updated.
            EnableInteraction();
        }
        ...
        internal override void UpdateBehavior(ChartBehavior oldBehavior, ChartBehavior newBehavior)
        {
            if (newBehavior != null || oldBehavior != null)
            {
                // Need to fix the tooltip and selection Interaction, temporary fix for WinUI-830184 WinUI Crashed issue when mouse hover on the series while data points dynamically updated.
                EnableInteraction();
            }

            base.UpdateBehavior(oldBehavior, newBehavior);
        }
        ...
        {
            // Need to fix the tooltip and selection Interaction, temporary fix for WinUI-830184 WinUI Crashed issue when mouse hover on the series while data points dynamically updated.
            if (EnableTooltip || SelectionBehavior != null)
            {
