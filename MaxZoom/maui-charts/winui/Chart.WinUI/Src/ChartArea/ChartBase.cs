...
#region Private Static Methods
        ...
        internal static void OnBehaviorChanged(DependencyObject d, DependencyPropertyChangedEventArgs args)
        {
            ...
            { 
                // Need to ensure, temporary fix for WinUI - 830184 WinUI Crashed issue when mouse hover on the series while data points dynamically updated.
                if (chartBase.VisibleSeries != null && chartBase.VisibleSeries.Count > 0)
                {
                    foreach (var series in chartBase.VisibleSeries)
                    {
                        if (series != null && series is FastLineSeries)
                        {
                            FastLineSeries fastLineSeries = (FastLineSeries)series;
                            fastLineSeries.EnableInteraction();
                        }
                    }
                }
        ...
        {
#if WinUI_Desktop
            if (!isRenderSeriesDispatched && DispatcherQueue != null)
            {
