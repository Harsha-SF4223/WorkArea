...
namespace Syncfusion.Maui.Charts
{
    ...
    public class RangeColumnSegment : ColumnSegment
    {
        ...
        internal string[] DataLabel = new string[2];
        ...
        private void UpdateDataLabels(PointF highPoint, PointF lowPoint)
        {
            for (int i = 0; i < 2; i++)
            {
               if(Series is RangeColumnSeries series)
                {
                    ...
                    if (DataLabels != null && DataLabels.Count > i)
                    {
                        ...
                        if (i == 0)
                        {
                            LabelPositionPoints[i] = series.IsDataInVisibleRange(xValue, y1) && !Empty ? dataLabelSettings.CalculateDataLabelPoint(series, this, highPoint, dataLabelSettings.LabelStyle, "HighType") : new PointF(float.NaN, float.NaN);

                        }
                        else if(i ==1)
                        {
                            LabelPositionPoints[i] = series.IsDataInVisibleRange(xValue, y2) && !Empty ? dataLabelSettings.CalculateDataLabelPoint(series, this, lowPoint, dataLabelSettings.LabelStyle, "LowType") : new PointF(float.NaN, float.NaN);
                        }

                        dataLabel.XPosition = LabelPositionPoints[i].X;
                        dataLabel.YPosition = LabelPositionPoints[i].Y;
                    }
                }
            }
        }
