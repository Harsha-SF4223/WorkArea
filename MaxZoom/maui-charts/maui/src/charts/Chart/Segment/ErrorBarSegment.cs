    ...
    public class ErrorBarSegment : ChartSegment
    {
        ...
        internal override void SetData(IList xList, IList yList)
        {
            ...
            if (Series is ErrorBarSeries errorBarSeries)
            {
                ...
                {
                    horizontalErrorValue = !errorBarSeries.HorizontalErrorValues.Any() ? 0 : errorBarSeries.HorizontalErrorValues!.Max();
                    verticalErrorValue = !errorBarSeries.VerticalErrorValues.Any() ? 0 : errorBarSeries.VerticalErrorValues!.Max();
                }
                else
                {
                ...
                if (errorBarSeries.Type == ErrorBarType.Percentage)
                {
                    double topVerticalPointMinY = topPointCollection.Select(p => p.Y).Min();
                    double topVerticalPointMaxY = topPointCollection.Select(p => p.Y).Max();
                    double bottomVerticalPointMinY = bottomPointCollection.Select(p => p.Y).Min();
                    double bottomVerticalPointMaxY = bottomPointCollection.Select(p => p.Y).Max();
