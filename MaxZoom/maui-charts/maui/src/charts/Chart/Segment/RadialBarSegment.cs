...
namespace Syncfusion.Maui.Charts
{
    ...
    public class RadialBarSegment : ChartSegment
    {
        ...
        protected internal override void OnLayout()
        {
            ...
            float angleDifference = (float)((float)(currentSeries.GetAngleDifference()) - ((currentSeries.EndAngle > currentSeries.StartAngle) ?  0.01 : -0.01));
            ...
            isCircularBar = Math.Round(currentSeries.EndAngle - currentSeries.StartAngle - 0.0001, 1).Equals(360) ||
                Math.Round(currentSeries.EndAngle - currentSeries.StartAngle - 0.0001, 1).Equals(-360);
            isTrack = true;
            ...
            isCircularBar = Math.Round(drawEndAngle - drawStartAngle - 0.0001, 1).Equals(360) || 
                Math.Round(drawEndAngle - drawStartAngle - 0.0001, 1).Equals(-360);
        ...
        private void DrawTrack(ICanvas canvas)
        {
            ...
            isCircularBar = Math.Round(currentSeries.EndAngle - currentSeries.StartAngle - 0.0001, 1).Equals(360) ||
                Math.Round(currentSeries.EndAngle - currentSeries.StartAngle - 0.0001, 1).Equals(-360);
            var disableTrack = (Math.Abs(trackEndAngle - trackStartAngle) <= 15);
        ...
        private void CapStyleCalculation(float midRadius, float segmentRadius, float startCurveAngle, float drawStartAngle, float drawEndAngle)
        {
            ...
            else if (currentSeries != null)
            {
                ...
                var endAngle = segmentEndAngle;
                if (currentSeries.StartAngle <= currentSeries.EndAngle) 
                {
                    segmentStartAngle = endAngle < startAngle ? endAngle : startAngle;
                    segmentEndAngle = endAngle < startAngle ? startAngle : endAngle;
                }
                else 
                {
                    segmentStartAngle = endAngle < startAngle ? startAngle : endAngle;
                    segmentEndAngle = endAngle < startAngle ? endAngle : startAngle;
                }
            }
        ...
        {
            if (currentSeries == null || double.IsNaN(YValue))
            {
                return;
            }
