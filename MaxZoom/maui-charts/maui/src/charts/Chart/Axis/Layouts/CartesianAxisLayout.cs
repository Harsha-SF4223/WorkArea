        ...
        private void UpdateActualAxis(ReadOnlyObservableCollection<ChartSeries> visibleSeries)
        {
            foreach (CartesianSeries series in visibleSeries)
            {
                if (series != null)
                {
                    ...
                    {
                        var axes = Area.IsTransposed ? VerticalAxes : HorizontalAxes;
                        var xName = series.XAxisName;
                        var axis = string.IsNullOrEmpty(xName) ? Area.PrimaryAxis : (GetAxisByName(xName, axes) ?? Area.PrimaryAxis);
                    ...
                    {
                        var axes = Area.IsTransposed ? HorizontalAxes : VerticalAxes;
                        var yName = series.YAxisName;
                        var axis = string.IsNullOrEmpty(yName) ? Area.SecondaryAxis : (GetAxisByName(yName, axes) ?? Area.SecondaryAxis);
