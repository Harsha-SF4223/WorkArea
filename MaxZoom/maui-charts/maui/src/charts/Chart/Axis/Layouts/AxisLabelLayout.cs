...
namespace Syncfusion.Maui.Charts
{
    internal class AxisLabelLayout
    {
        ...
        internal virtual SizeF Measure(SizeF availableSize)
        {
            ...
            if (NeedToRotate)
            {
                ComputedSizes = new List<SizeF>();//TODO: Need of creating compute size
            }
            ...
            if (Axis != null && Axis.VisibleLabels != null && Axis.VisibleLabels.Count > 0)
            {
                foreach (var axisLabel in Axis.VisibleLabels)
                {
                    ...
                    Size measuredSize = GetLabelSize(label, labelStyle);
                    Size desiredSize = new Size(measuredSize.Width + MarginLeft + MarginRight, measuredSize.Height + MarginTop + MarginBottom);
        ...
        private static Size GetRotatedSize(Size size, double degrees)
        {
    ...
    internal class VerticalLabelLayout : AxisLabelLayout
    {
        ...
        private float CalculatePolarPosition(float coeff, float width, float height, float size)
        {
