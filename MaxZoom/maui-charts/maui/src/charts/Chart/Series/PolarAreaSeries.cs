...
namespace Syncfusion.Maui.Charts
{
    ...
    /// 
    /// <para> <b>EnableTooltip - </b> A tooltip displays information while tapping or mouse hovering above a segment. To display the tooltip on a chart, set the <see cref="ChartSeries.EnableTooltip"/> property as <b>true</b> in <see cref="PolarAreaSeries"/> class, and also refer <seealso cref="ChartBase.TooltipBehavior"/> property.</para>
    ...
    public class PolarAreaSeries : PolarSeries
    {
        ...
        internal override void GenerateSegments(SeriesView seriesView)
        {
            ...
            for (int i = 0; i < PointsCount; i++)
            {
                ...
                if (double.IsNaN(YValues[i]) || i == PointsCount - 1)
                {
                    if (xValues != null)
                    {
                        var segment = CreateSegment() as PolarAreaSegment;
                        if (segment != null)
                        {
                            segment.Series = this;
        ...
        internal override void DrawDataLabels(ICanvas canvas)
        {
            ...
            PolarAreaSegment? datalabel = Segments[0] as PolarAreaSegment;

            if (datalabel == null || xValues == null || YValues == null) return;
