...
namespace Syncfusion.Maui.Charts
{
    ...
    public abstract class PolarSeries : ChartSeries, IDrawCustomLegendIcon, IMarkerDependent
    {
        ...
        /// </summary>
        /// <value>If its <c>true</c>, series path will be closed; otherwise opened.</value>
        /// <example>
        ...
        {
            SizeF labelSize = LabelTemplate == null ? labelStyle.MeasureLabel(dataLabel.LabelContent != null ? dataLabel.LabelContent : string.Empty) : GetLabelTemplateSize(dataLabel);
        ...
        internal float SumOfValues(IList<double> Yvalues)
        {
            float sum = 0f;
        ...
        internal override void InvalidateSeries()
        {
            if (ChartArea != null)
            {
                foreach (var seriesView in ChartArea.SeriesViews.Children)
                {
                    if (seriesView is SeriesView view && view.Series == this)
                    {
        ...
        private void InvalidatePolarDataLabel()
        {
            ChartArea?.DataLabelView?.InvalidateDrawable();
        }

        private PointF TransformVisiblePoint(double xValue, double yValue)
        {
            return TransformVisiblePoint(xValue, yValue, 1);
        }

        private int GetTouchPointIndex(float x, float y)
        {
        ...
        private List<float> GetMidPoints()
        {
        ...
        private PointF CalculateDataLabelInnerPosition(ChartSegment dataLabel, SizeF labelSize, PointF labelPoint, float padding)
        {
        ...
        private PointF CalculateDataLabelOuterPosition(ChartSegment dataLabel, SizeF labelSize, PointF labelPoint, float padding)
        {
            if (ChartArea == null) return labelPoint;
        ...
        private static void OnIsClosedChanged(BindableObject bindable, object oldValue, object newValue)
        {
            if (bindable is PolarSeries series)
            {
        ...
        private void DrawMarker(ICanvas canvas, int index, ShapeType type, Rect rect)
        {
