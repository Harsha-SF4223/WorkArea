...
namespace Syncfusion.Maui.Charts
{
    internal class CartesianChartArea : AreaBase, ICartesianChartArea
    {
        ...
        private Dictionary<string, List<StackingSeriesBase>>? seriesGroup;
        #endregion
        ...
        internal Element SourceParent { get; private set; }
        ...
        public CartesianChartArea(SfCartesianChart chart)
        {
            ...
            behaviorLayout = chart.BehaviorLayout;
        ...
        protected override void UpdateAreaCore()
        {
            ...
            cartesianPlotArea.Margin = PlotAreaMargin;
            behaviorLayout.Margin = PlotAreaMargin;
        ...
        internal void CalculateSbsPosition()
        {
            ...
            foreach (var cartesianSeries in cartesianSeriesCollection)
            {
                ...
                foreach (CartesianSeries xAxisRegSeries in cartesianSeries.ActualXAxis.RegisteredSeries)
                {
                        ...
                        if (!xAxisRegSeries.IsSbsValueCalculated && xAxisRegSeries.ActualXAxis != null)
                        {
                            ...
                            if (xAxisRegSeries is StackingSeriesBase baseSeries)
                            {
                                ...
                                foreach (var yAxisRegSeries in baseSeries.ActualYAxis.RegisteredSeries)
                                {
                                    ...
                                    {
                                        if (!stackingSeries.IsSbsValueCalculated && seriesGroup != null)
                                        {
                                            string groupID = seriesGroup.FirstOrDefault(x => x.Value.Any(s => s.GroupingLabel == stackingSeries.GroupingLabel && s.GetType()==stackingSeries.GetType())).Key;
        ...
        internal void UpdateStackingSeries()
        {
            ...
            seriesGroup = new Dictionary<string, List<StackingSeriesBase>>();
            foreach (var series in VisibleSeries)
            {
                if (series is StackingSeriesBase stackingSeries && stackingSeries.IsVisible)
                {
                    ...
                    if (seriesGroup.ContainsKey(stackingGroup))
                    {
                        ...
                        {
                            string key = seriesGroup.FirstOrDefault(x => x.Value.Any(y => 
                                            y.GetType().Name == stackingSeries.GetType().Name &&
                                            y.GroupingLabel == "" && 
                                            y.ActualYAxis!.RegisteredSeries.Contains(stackingSeries))).Key;

                            seriesList = key != null ? seriesGroup[key] : new List<StackingSeriesBase>();
                        }
                    ...
                    foreach (var series in seriesList)
                    {
                        ...
                        for (int i = 0; i < xValues.Count; i++)
                        {
                            ...
                            if (yValue >= 0)
                            {
                                ...
                                {
                                    bottomValues.Add((axisCross > positiveYValues[xValue]) ? axisCross : positiveYValues[xValue]);
                                    if (series.GetType().Name.Contains("Stacking") && series.GetType().Name.Contains("100Series"))
                                    {
        ...
        private static double GetYValue(List<StackingSeriesBase> SeriesList, double yValue, int index)
        {
